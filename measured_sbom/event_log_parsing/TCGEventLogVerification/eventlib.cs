...
namespace TCGEventLogVerification
{
    ...
    // For some reason, event logs can have multiple digest. The TCG specs speciy how to read and parse them
    // Typically, there are sha1, sha256, sha384.
   ...
   // this class is where event are actually parsed. Each event type can be their own structures.
    ...
    {   
        ...
        private Dictionary<string,byte[]> rim ; 


        public static TcgEvent2 ReadFrom(BinaryReader reader, TcgEventHeader2 header)
        {
            ...
            event2.ParseEventData(header);

            return event2;
        }


        private void ParseEventData(TcgEventHeader2 header)
     
        {   // This encodes knowledge about events that were obtained from reading tcg specs. 
            // This part is meant to be a framework to expand in the future. Each event needs is unique
            // how it is parsed and verified. 

            // High level idea is to read in the reference integrity manifest which we are modeling as dictionary of name and digest.  There are 
            // things are signature verification which we are abstracting now.  For an event, we look for payload name or variable name in reference
            // integrity manifiest, compare the digest. 
            switch (EventType)
            {
                ...
                case EventType.EV_POST_CODE:
                    ...
                    // Assume 'rim' is a Dictionary<string, byte[]> containing  RIM entries.
                         ...
                         if (rim.TryGetValue("POST_Code_Module", out expectedDigest))
                         {
                             ...
                             foreach (var digest in header.Digests)
                             {
    ...
    public class TcgEventHeader2
    {
        ...
        public static TcgEventHeader2 ReadFrom(BinaryReader reader)
        {
            ...
            header.Event = TcgEvent2.ReadFrom(reader, header);

            return header;
        }
    }
   // This is entry point into event parser. 
   public class TcgEventLogParser
    { 
        public static void ParseEventLog(string filePath)
        {
            try
            {
                ...
                {
                    // Read the first TCG_EVENT. The first event is in different format from 
                    // all other subsequent event. The purpose of first event is to signify starts of the log. 
                    // nothing should be in digest. 
                    TcgEvent firstEvent = TcgEvent.ReadFrom(reader);
                    DisplayEvent(firstEvent);

                    // All subsequent events are parsed by here. TcgEventHeader2 is outermost parser, it breaks an events into 
                    // the main parts. From here specific parser for digest and event type are called.
        ...
        static void DisplayEventHeader2(TcgEventHeader2 headerEvent)
        {
            ...
            if ((headerEvent.Event.EventType == EventType.EV_EFI_VARIABLE_DRIVER_CONFIG || headerEvent.Event.EventType == EventType.EV_EFI_VARIABLE_BOOT) && headerEvent.Event.UefiVariableData != null)
            {
